...
使用本地缓存：避免带宽或者传输影响，本地缓存热点key数据，对于每次读请求，将⾸先检查key是否存在于本地缓存中，如果存在则直接返回，如果不存在再去访问分布式缓存的机器  。
...
- redis-cli redis 客户端
- 启动可以指定配置文件，在redis加压目录有个redis.conf 或者其他配置文件
...
//hubmirror.c.163.com","https://docker.mirrors.ustc.
edu.cn"]
} 
