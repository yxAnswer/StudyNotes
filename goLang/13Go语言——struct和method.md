...
使用值来调用此方法，receiver是 Datal类型。当发起调用的时候，此方法操作的是值调用者的一个值的拷贝。如最上面的代码中，连续值调用最终打印出的地址是不同的，每次都不相同，因为每一次的调用，都是使用的值的拷贝，并且对这个值做任何操作，都不会影响初始值。
...
使用指针来调用此方法，receiver是一个指向Data类型的指针。当调用使用指针接收者声明的方法时，这个方法会共享调用方法时接收者所指向的值。其实也就是，此方法直接可以操作接收者指针指向的底层数据的值。这样方法对值进行任何改变都会对原始值产生影响。 类似引用传递。
...
不能讲一个指针变量去调用指针接受者的方法，只能用这个接受者的指针去调用。
