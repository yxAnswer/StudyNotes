...
其实使用这种组合的方式，可以将不同的结构体组成一个集合。

<<<<<<< HEAD
小结，struct是值类型，赋值和传参会复制全部内容。可⽤ "_" 定义补位字段，⽀持指向自⾝类型的指针
=======
struct是值类型，赋值和传参会复制全部内容。可⽤ "_" 定义补位字段，⽀持指向自身类型的指针
>>>>>>> 3dab0c3cba94ed8e65947c6391c68131eb9bd7f6
...
receiver 声明的是谁，这个就是哪个类型的方法。注意：只能为同一个包中的类型定义方法
...
go是直接支持struct的封装的，go语言的可见性是根据首字母大小写来实现的。 首字母大写表示对外部可见，等同Java中的public，首字母小写，对外部不可见，等同于Java中的private。
